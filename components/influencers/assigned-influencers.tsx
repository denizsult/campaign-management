...
export function AssignedInfluencers({ campaignId }: AssignedInfluencersProps) {
  const { data: assignedInfluencers, isLoading } =
  ...
  const isEmpty =
    !isLoading && (!assignedInfluencers || assignedInfluencers.length === 0);
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">
              Assigned Influencers ({assignedInfluencers?.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Badge
                          ...
                          className={getEngagementColor(
                            Number(influencer.engagement_rate)
                          )}
                        >
